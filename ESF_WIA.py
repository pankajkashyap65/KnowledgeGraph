...
# In[2]: change as per your directory installation
...
# performs community identification in the entity Graph
...
#    lst = list(comp)
    ...
    for idx, val in enumerate(softmax_list):
        ...
        for word in cls:
            if vec is not None:
                vec = np.add(vec, transe_vec_for_entity(word))
            else:
                vec = transe_vec_for_entity(word)
            vec = vec / len(cls) #centroid of cluster
        if val > mean:
            print("cluster number: ", idx, " Selected")
#            cls = next_community[idx]
            for word in cls:
                print("ent_id: ", word, " : ", id2title.get(word))
                ent_vec = transe_vec_for_entity(word)
                weight = 0.0
                cluster_weight = gaussian(distance.euclidean(ent_vec, vec), sig=sig)
                query_similarity = gaussian(distance.euclidean(ent_vec, qcluster_centre), sig=sig)
                weight = (cluster_weight + query_similarity)/2
...
#                print("adding link_title: ", link_title, " and wiki_id: ", wiki_id)
...
#    Path(path).mkdir(parents=True, exist_ok=True)
    ...
    file_name = "query_G/prob_dist_ESF_Softmax/"+dataset+"/"+ dataset+"_ti"
    file1 = open(file_name, "a+")
#    if dataset == "cw12" or dataset == "msmarco" or dataset == "trec678":
